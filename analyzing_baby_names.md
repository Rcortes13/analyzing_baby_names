...
<p>I wanted to find a traditionally female name ending in the letter 'a' since apparently vowels in baby names are trendy. Also I looked for a name that has been popular in the years since 2015. </p>
...
<p>Noah and Liam have ruled the roost in the last few years, but if we scroll down in the results, it looks like Michael and Jacob have also spent a good number of years as the top name!</p>


## 8. The most years at number one
 <p>Which name has been number one for the largest number of years? I'll use a common table expression to find out. </p>


```sql

WITH count_top_name AS (
SELECT a.year, a.first_name, a.num
FROM baby_names a
INNER JOIN (SELECT year,
       MAX(num) AS max_num
